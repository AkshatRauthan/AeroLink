...
const READ_REPLICA_ENABLED = boolean('PAYMENT_READ_REPLICA_ENABLED', false);
const PAYMENT_SHARD_COUNT = positiveInteger('PAYMENT_SHARD_COUNT', '1');
...
const DbConfig = {
    ...
    getPaymentShardEnvironment: (shardIndex: number): PaymentShardEnvironment => {
        ...
        const primary: PaymentDatabaseEnvironment = {
            ...
            user: required('PAYMENT_DB_USER', 'auth_service'), password: required('PAYMENT_DB_PASSWORD', 'auth_service_password'),
        ...
        return {
            primary, replica: {
                ...
                user: required('PAYMENT_REPLICA_DB_USER', primary.user), password: required('PAYMENT_REPLICA_DB_PASSWORD', primary.password),
